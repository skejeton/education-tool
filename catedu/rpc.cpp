...
static RpcCallPacket
read_rpc_call_packet(void* data)
{
    size_t offset = 0;
    ...
    return { (const char*)data, broadcast, client, data_size, data_value };
}

static RpcRawPacket
write_rpc_call_packet(RpcCallPacket data)
{
    size_t function_name_len = strlen((const char*)data.function_name) + 1;
    auto* raw_data =
      (uint8_t*)malloc(function_name_len + data.data_size + sizeof(size_t) +
                       sizeof(bool) + sizeof(RpcClient));
    assert(raw_data);
...
static bool
is_function_registered(Rpc* rpc, const char* name)
{
...
static void
call_rpc_function(Rpc* rpc, RpcCallPacket packet)
{
    RpcCallbackContext context =
      rpc->functions[std::string(packet.function_name)];

    context.callback(
      rpc, packet.client, context.userdata, packet.data_size, packet.data);
}

bool
Rpc::host(Rpc* out_rpc, ENetAddress address)
{
    Rpc rpc = {};

    rpc.enet_host =
      enet_host_create(&address, DEFAULT_CLIENT_CAP, DEFAULT_CHAN_CAP, 0, 0);
...
bool
Rpc::connect(Rpc* out_rpc, ENetAddress address)
{
    Rpc rpc = {};
    ...
    rpc.enet_peer =
      enet_host_connect(rpc.enet_host, &address, DEFAULT_CHAN_CAP, 0);
    ...
    /*
    ENetEvent event;
    if (!(enet_host_service(rpc.enet_host, &event, 5000) > 0 && event.type ==
    ENET_EVENT_TYPE_CONNECT)) { enet_peer_reset(rpc.enet_peer);
        enet_host_destroy(rpc.enet_host);
        return false;
    }
    */
    ...
    while (enet_host_service(this->enet_host, &event, 0) > 0) {
        switch (event.type) {
            case ENET_EVENT_TYPE_CONNECT:
                ...
                if (is_server()) {
                    (void)broadcast(
                      "_connect", 0, nullptr, event.peer->connectID);
                }
                break;
            case ENET_EVENT_TYPE_DISCONNECT:
                ...
                if (!is_server()) {
                    call_rpc_function(this,
                                      { "_disconnect", false, 0, 0, nullptr });
                    this->connected = false;
                } else {
                    (void)broadcast(
                      "_disconnect", 0, nullptr, event.peer->connectID);
                }
                break;
            case ENET_EVENT_TYPE_RECEIVE:
                printf("Receive %d\n", is_server());
                {
                    RpcCallPacket packet =
                      read_rpc_call_packet(event.packet->data);

                    if (is_server()) {
                        if (packet.broadcast) {
                            (void)broadcast(packet.function_name,
                                            packet.data_size,
                                            packet.data);
                        } else {
                            (void)send(packet.client,
                        ...
                        assert(
                          is_function_registered(this, packet.function_name));
...
bool
Rpc::broadcast(const char* fn_name,
               ...
               uint32_t poser)
{
    ...
    RpcCallPacket call_packet = { fn_name,
                                  true,
                                  is_server() ? poser
                                              : this->enet_peer->connectID,
                                  data_size,
                                  data };
    RpcRawPacket raw_packet = write_rpc_call_packet(call_packet);

    ENetPacket* packet = enet_packet_create(
      raw_packet.data, raw_packet.size, ENET_PACKET_FLAG_RELIABLE);
...
void
Rpc::register_function(const char* fn_name,
                       void* userdata,
