...
void
show_file_list(MainMenu* main_menu, ImVec2 size)
{
...
void
synchronize_file_list(MainMenu* main_menu)
{
    main_menu->file_selected = 0;
    main_menu->files = {};

    for (const auto& entry :
         std::filesystem::directory_iterator(main_menu->directory_path)) {
        main_menu->files.push_back(entry.path().filename().generic_string());
    }
}

std::string
get_selected_file_filename(MainMenu* main_menu)
{
    return main_menu->files[main_menu->file_selected];
}

void
remove_selected_file(MainMenu* main_menu)
{
    if (main_menu->files.size() > 0) {
        std::filesystem::remove(main_menu->directory_path /
                                get_selected_file_filename(main_menu));
        synchronize_file_list(main_menu);
    }
}

void
create_file(MainMenu* main_menu, const char* filename)
{
    if (filename[0] == 0) {
        return;
    }
...
void
open_selected_file(MainMenu* main_menu, OpenProject* project)
{
    project->path =
      (main_menu->directory_path / get_selected_file_filename(main_menu))
        .string();
    project->is_open = true;
}

void
host_selected_file(MainMenu* main_menu, OpenProject* project)
{
...
void
join_selected_ip(MainMenu* main_menu, OpenProject* project)
{
    project->host_name = main_menu->host_name;
...
MainMenu
MainMenu::init(std::filesystem::path directory_path)
{
    MainMenu menu = {};
...
bool
MainMenu::show(OpenProject* project)
{
    bool opened = false;
    ...
    if (ImGui::Begin("Main Menu v1")) {
        ...
        ImGui::InputTextWithHint(
          "##Host", "Host", this->host_name, MENU_STRING_SIZE);
        ...
        ImGui::SetNextItemWidth(150);
        ImGui::InputTextWithHint(
          "##File Name", "File Name", this->file_name, MENU_STRING_SIZE);
