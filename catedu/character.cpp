...
void
Character::draw(BoxdrawRenderer* renderer, Vector4 color_multiple)
{
    ...
    Box3 parts[PART_COUNT] = {
        ...
        box3_extrude_from_point(position + Vector3{ 0, 1.5, 0 },
                                { 0.7, 0.7, 0.7 }),
        box3_extrude_from_point(position + Vector3{ 0, 0.4, 0.4 },
                                { 0.4, 0.6, 0.4 }),
        box3_extrude_from_point(position + Vector3{ 0, 0.4, -0.4 },
                                { 0.4, 0.6, 0.4 }),
    };
    ...
    for (size_t i = 0; i < PART_COUNT; i++) {
        boxdraw_push(renderer,
                     boxdraw_cmdgradient(parts[i], COLOR_TOP, COLOR_BOTTOM));
    }
}
