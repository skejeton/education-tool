...
)
{
    ...
    return rendering_2d_generate_mesh_centered_function(desc, sample_count, 0, MATH_TAU, generator);
}

RenderGeo rendering_2d_generate_circle(RenderWriteDesc desc, size_t sample_count)
{
    auto generator = [](float theta){
        return Vector2{cos(theta)*0.5f, sin(theta)*0.5f};
    };
...
RenderGeo rendering_2d_generate_square(RenderWriteDesc desc)
{
    ...
    generator.push_tri(a, c, b);
    generator.push_tri(b, d, c);
    generator.push_tri(b, c, a);
    generator.push_tri(c, b, d);
