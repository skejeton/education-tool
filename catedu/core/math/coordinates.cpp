...
{
    return {sqrt(coord.x*coord.x + coord.y*coord.y), atan2(coord.y, coord.x)};
}

Vector3 polar3_into_cartesian(Polar3 coord)
{
    return {
        coord.radius*cos(coord.theta)*sin(coord.phi),
...
Polar3 cartesian_into_polar3(Vector3 coord)
{
    return {
        ...
        acos(coord.z/sqrt(coord.x*coord.x + coord.y*coord.y + coord.z*coord.z))
    };
}
