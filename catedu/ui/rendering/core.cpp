...
{
    return ui_image_make_from_data(
      { (size_t)(width * height * 4), (void*)data }, { width, height });
}
...
generate_test_image()
{
    return generate_image_from_data(
      2, 2, "\xFF\x00\xFF\xFF\x00\x00\x00\xFF\x00\x00\x00\xFF\xFF\x00\xFF\xFF");
}
...
UiRenderingCore::init()
{
    ...
    result.buffers[(int)UiBuffers::Squircle] =
      generate_buffer(rendering_2d_generate_squircle(
        { 4, 0 }, DEFAULT_SAMPLE_COUNT, 8, 0.5, 0.5));
...
void
UiRenderingCore::begin_pipeline()
{
    this->pip_size = { sapp_widthf(), sapp_heightf() };

    sg_begin_default_pass(
      &this->pass_action, this->pip_size.x, this->pip_size.y);
    sg_apply_pipeline(this->pipeline);
}

void
UiRenderingCore::end_pipeline()
{
    sg_end_pass();
}

UiImageId
UiRenderingCore::alloc_image(UiImage image)
{
    return this->images.allocate(image);
}

void
UiRenderingCore::dealloc_image(UiImageId id)
{
...
UiImage*
UiRenderingCore::get_image(UiImageId id)
{
    return this->images.get(id);
}

void
UiRenderingCore::render_object(UiBrush brush)
{
