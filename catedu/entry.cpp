...
static void
save_entity_in_editor(Entry* entry)
{
    entry->entity_editor.emplace(&entry->nc);
}

static void
set_mode(Entry* entry, PlayingMode mode)
{
    if (mode == PLAYING_MODE_MENU) {
        ...
        entry->main_menu =
          MainMenu::init(std::filesystem::current_path() / "user");
        return;
    }
...
static void
handle_game_mode_input(Entry* entry, Input* inputs)
{
...
static Player*
get_player(Entry* entry)
{
    return entry->env.player_pool.players.get(entry->nc.player_id);
}

static bool
is_game_mode(PlayingMode playing_mode)
{
    return playing_mode == PLAYING_MODE_BUILD ||
           playing_mode == PLAYING_MODE_PLAY;
}

static void
handle_in_game_input(Entry* entry, Input* inputs)
{
    Player* player = get_player(entry);

    switch (entry->env.playing_mode) {
        case PLAYING_MODE_PLAY:
            if (inputs->key_states[SAPP_KEYCODE_SPACE].pressed &&
                player->camera.position.y < 2.1) {
...
static void
handle_input(Entry* entry, Input* inputs)
{
...
void
check_collisions(Entry* entry)
{
    Player* player = get_player(entry);

    Rect camera_rect = { { player->camera.position.x - 1,
...
static void
init_imgui_font(Entry* entry, const char* path, float size)
{
    auto io = ImGui::GetIO();
    entry->main_font =
      io.Fonts->AddFontFromFileTTF(path, sapp_dpi_scale() * size);
    uint8_t* font_pixels;
...
const char*
selection_option_name(SelectionOption selection)
{
    switch (selection) {
...
static Entity
generate_entity_from_type(SelectionOption selection,
                          Vector3 position,
                          int height)
{
    Entity ent = {};
...
void
put_information_window(InformationWindowData data)
{
...
int
input_inventory_index(Input input)
{
    if (input.key_states[SAPP_KEYCODE_1].pressed)
        return 0;
    if (input.key_states[SAPP_KEYCODE_2].pressed)
        return 1;
    if (input.key_states[SAPP_KEYCODE_3].pressed)
        return 2;
    if (input.key_states[SAPP_KEYCODE_4].pressed)
        return 3;
    if (input.key_states[SAPP_KEYCODE_5].pressed)
        return 4;
    if (input.key_states[SAPP_KEYCODE_6].pressed)
        return 5;
    if (input.key_states[SAPP_KEYCODE_7].pressed)
        return 6;
    if (input.key_states[SAPP_KEYCODE_8].pressed)
        return 7;
    if (input.key_states[SAPP_KEYCODE_9].pressed)
        return 8;
    return -1;
}

SelectionOption
input_selection_option(Input input, SelectionOption current)
{
...
void
render_selection_box_base(BoxdrawRenderer* renderer, Box3 box)
{
...
void
render_entity(BoxdrawRenderer* renderer,
              Entity* entity,
              bool selected,
              bool hovered)
{
...
show_ui_game_mode(Entry* entry)
{
    ...
    switch (entry->menu) {
        case HELP:
            ...
            {
                gui.label("dialog count: %zu",
                          entry->env.flashbacks.dialogs.count);
                TableIterator iter = TableIterator<FlashbacksDialog>::init(
                  &entry->env.flashbacks.dialogs);
                for (; iter.going(); iter.next()) {
                    gui.label("%zu/%zu", iter.id.id, iter.id.generation);
                }
            }
            {
                gui.label("player count: %zu",
                          entry->env.player_pool.players.count);
                TableIterator iter =
                  TableIterator<Player>::init(&entry->env.player_pool.players);
                for (; iter.going(); iter.next()) {
                    gui.label("%zu/%zu", iter.id.id, iter.id.generation);
                }
            }
...
static void
draw_background(ImU32 color)
{
    ImDrawList* draw_list = ImGui::GetBackgroundDrawList();
    draw_list->AddRectFilled(
      { 0, 0 }, { sapp_widthf(), sapp_heightf() }, color);
}
...
handle_game_mode(Entry* entry)
{
    ...
    entry->selection_option =
      input_selection_option(entry->inputs, entry->selection_option);
    ...
    boxdraw_push(&entry->boxdraw,
                 boxdraw_cmdcolored(floor, { 0.1, 0.8, 0.15, 1.0 }));

    auto pointing_at = PointingAtResolve::init(player->camera.ray());
    auto pointing_at_bindings = TempIdBinder<TableId>::init();

    if (entry->env.playing_mode == PLAYING_MODE_BUILD) {
        PlacementGrid grid = PlacementGrid::init(player->camera.position.x,
                                                 player->camera.position.z);

        for (SceneIterator iterator = scene_iterator_begin(&entry->env.scene);
             scene_iterator_going(&iterator);
             scene_iterator_next(&iterator)) {
            grid.place_region(entity_placement_region(
              scene_get_entity(&entry->env.scene, iterator.id)));
        }
        ...
        Entity potential_entity = generate_entity_from_type(
          entry->selection_option, { (float)rx, 0, (float)ry }, entry->bheight);

        bool can_place =
          may_place &&
          grid.try_place_region(entity_placement_region(&potential_entity));
            ...
            if (entry->env.playing_mode == PLAYING_MODE_PLAY) {
                ...
                if (entry->inputs.mouse_states[0].released) {
                    ...
                    if (entry->entity_selected.id == id) {
                        ...
                        entry->entity_editor =
                          EntityEditor::init(&entry->env.flashbacks);
                    }
    ...
    // Draw all players.
    {
        ...
        for (; iter.going(); iter.next()) {
            ...
            if (iter.id != entry->nc.player_id) {
                Character{
                    iter.table->get(iter.id)->camera.position -
                    Vector3{ 0, 1, 0 }
                }.draw(&entry->boxdraw, { 1.0, 1.0, 0, 1.0 });
            }
        }
    }
    ...
    switch (entry->env.playing_mode) {
        case PLAYING_MODE_PLAY: {
            ...
            player->camera.position +=
              player->camera_velocity * sapp_frame_duration();
...
Entry::input(const sapp_event* event)
{
    if (!simgui_handle_event(event)) {
        ...
        if (!sapp_mouse_locked() && event->type == SAPP_EVENTTYPE_MOUSE_UP &&
