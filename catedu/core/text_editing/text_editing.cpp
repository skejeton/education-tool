...
static size_t remaining_space(SingleLineTextEditor &editor)
{
...
static SingleLineTextCursor sanitize_cursor(SingleLineTextEditor &editor,
...
size_t SingleLineTextEditor::insert(const char *str)
{
    ...
    memmove(insertion_point + characters_to_insert, insertion_point,
...
size_t SingleLineTextEditor::erase_back()
{
    ...
    memmove(deletion_point, deletion_point + 1,
            this->buffer.size - this->cursor.column);
...
void SingleLineTextEditor::move_cursor(int delta)
{
...
void SingleLineTextEditor::set_cursor(SingleLineTextCursor cursor)
{
    // 1. Update the cursor.
